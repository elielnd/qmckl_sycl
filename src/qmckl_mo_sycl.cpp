    ...
    double *__restrict__ mo_vgl)
{
    ...
    q.submit([&](handler &h)
             { h.parallel_for(range<1>(point_num), [=](id<1> j)
                              {

                    // Set j subarray to 0
                    for (int64_t k = 0; k < 5; ++k)
                    {
                        for (int64_t l = 0; l < mo_num; l++)
                        {
                            ...
                            if(ao_vgl[k + ao_num * 5 * j] != 0.)
                            {
                                ...
                                for(int64_t i = 0; i < mo_num; i++)
                                {
                                    mo_vgl[i + mo_num * l + mo_num * 5 * j] = 
                                        mo_vgl[i + mo_num * l + mo_num * 5 * j] + coefficient_t[i + mo_num * k] * c1;
                                }
                            }
                        }
                    } }); });
...
qmckl_exit_code_device qmckl_compute_mo_basis_mo_value_device(
    qmckl_context_device context, int64_t ao_num, int64_t mo_num,
    int64_t point_num, double *__restrict__ coefficient_t, double *__restrict__ ao_value,
    double *__restrict__ mo_value)
{
    assert(context != QMCKL_NULL_CONTEXT_DEVICE);

	double *av1_shared = (double *)qmckl_malloc_device(context, point_num * ao_num * sizeof(double));
	int64_t *idx_shared =(int64_t *)qmckl_malloc_device(context, point_num * ao_num * sizeof(int64_t));
...
qmckl_finalize_mo_basis_device(qmckl_context_device context)
{
    ...
    q.submit([&](handler &h)
             {
                 h.parallel_for(range<2>(ao_num, mo_num), [=](id<2> idx)
                                {
                                    auto i = idx[0];
                                    auto j = idx[1];
