...
qmckl_exit_code_device qmckl_compute_jastrow_factor_ee_deriv_e_device(
    ...
    queue.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::range<3>(walk_num, 4, elec_num), [=](sycl::id<3> idx) {
            auto nw = idx[0];
            auto ii = idx[1];
            auto j = idx[2];
    ...
    queue.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::range<3>(walk_num, elec_num, elec_num), [=](sycl::id<3> idx) {
            auto nw = idx[0];
            auto i = idx[1];
            auto j = idx[2];
            const double x0 =
...
qmckl_compute_jastrow_factor_een_rescaled_e_deriv_e_device(
    ...
    queue.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::range<1>(elec_num * 4 * elec_num * (cord_num + 1) * walk_num), [=](sycl::id<1> idx) {
			auto i = idx[0];
