...
qmckl_exit_code_device qmckl_compute_mo_basis_mo_vgl_sgemm_device(
	...
	const double *__restrict__ coefficient_t, const double *__restrict__ ao_vgl,
	double *__restrict__ const mo_vgl) {
	...
	float *A = reinterpret_cast<float *>qmckl_malloc_device(context, sizeof(float) * ao_num * mo_num);
	float *B =
		reinterpret_cast<float *>qmckl_malloc_device(context, sizeof(float) * 5 * ao_num * point_num);
	float *C =
...
qmckl_exit_code_device qmckl_compute_mo_basis_mo_value_dgemm_device(
	const qmckl_context_device context, const int64_t ao_num,
	const int64_t mo_num, const int64_t point_num,
	const double *__restrict__ coefficient_t, const double *__restrict__ ao_value,
	double *__restrict__ const mo_value) {
	assert(context != QMCKL_NULL_CONTEXT_DEVICE);
