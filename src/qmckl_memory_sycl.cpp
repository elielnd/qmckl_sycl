...
#include "../include/qmckl_context_sycl.h"
                        ...
                        const qmckl_memory_info_struct_device info)
{
    ...
    try
    {
        ...
        if (ctx->memory.n_allocated == ctx->memory.array_size)
        {
            ...
            // memset is not needed with malloc_shared

            // Copy the old array to the new one
            sycl::memcpy(new_array, ctx->memory.element, old_size * sizeof(qmckl_memory_info_struct_device), queue).wait();
